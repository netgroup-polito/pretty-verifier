...
def handle_error(output_raw, c_source_files, bytecode_file, llvm_objdump=None):
    ...
    leaks_addr_as_return_value_pattern = re.search(r"R(\d+) leaks addr as return value", error)
    ...
    async_callback_register_not_known_pattern = re.search(r"In async callback the register R(\d+) is not a known value \((.*?)\)", error)
    ...
    subprogram_exit_register_not_scalar_pattern = re.search(r"At subprogram exit the register R(\d+) is not a scalar value \((.*?)\)", error)
    ...
    program_exit_register_not_known_pattern = re.search(r"At program exit the register R(\d+) is not a known value \((.*?)\)", error)
