...
// TODO INJECT capire
func NewBpfProg(pt *BpfProgType, r *randGen, opt BrfGenProgOpt, injector string) *BpfProg {
	p := &BpfProg{
		pt:       pt,
	...
	if opt.useTestSrc {
		...
		files, _ := os.ReadDir(opt.basePath)
		count := 0
		for _, f := range files {
		...
		p.BasePath = fmt.Sprintf("%v/prog_%03d%s", opt.basePath, count, injector)
	}
...
func getHelperCompatMaps(p *BpfProg, call *BpfCall) []*BpfMap {
	...
	for _, m := range p.Maps {
		if !injectGetHelperCompactMaps && !isMapFuncCompatible(m.Type, call.Helper.Enum) {
			continue
		}
		...
		//6084
		if !injectGetHelperCompactMaps && (call.Helper.Enum == BPF_FUNC_map_delete_elem || call.Helper.Enum == BPF_FUNC_map_update_elem ||
		...
		if !injectGetHelperCompactMaps && (genSpinlock && (!mapHasSpinlock || mapIsRdOnly)) ||
			(genTimer && (!mapHasTimer || mapIsRdOnly)) ||
			(genConstStr && !mapHasConstStr) {
			continue
		}
		//11473, 11478, 11484
		if !injectGetHelperCompactMaps && mapHasSpinlock &&
			(p.pt.Enum == BPF_PROG_TYPE_SOCKET_FILTER || isTracingProgType(p.pt.Enum) || p.Sec.Sleepable) {
			continue
		}
		//11503
		if !injectGetHelperCompactMaps && m.Type == "BPF_MAP_TYPE_STRUCT_OPS" {
			continue
		}
		//11526
		if !injectGetHelperCompactMaps && p.Sec.Sleepable &&
		...
		if m.Type == "BPF_MAP_TYPE_CGROUP_STORAGE" {
			...
			if !injectGetHelperCompactMaps && hasCgroupStorageMap {
				continue
			}
		}
		if m.Type == "BPF_MAP_TYPE_PERCPU_CGROUP_STORAGE" {
			...
			if !injectGetHelperCompactMaps && hasPercpuCgroupStorageMap {
				continue
			}
		}
		...
		if !injectGetHelperCompactMaps && call.Helper.Ret == "RET_PTR_TO_MAP_VALUE_OR_NULL" {
		...
		if !injectGetHelperCompactMaps && (call.Helper.Ret == "RET_PTR_TO_MAP_VALUE" || call.Helper.Ret == "RET_PTR_TO_MAP_VALUE_OR_NULL") && call.Hint.RetAccessSize != 0 {
...
func (p *BpfProg) genCompatibleRegTypes(call *BpfCall, arg int) ([]RegType, string) {
	...
	if injectGenCompatibleRegTypes {
		return compatibleRegType[argType], btfId
	}
...
func (p *BpfProg) genRandBpfCtxAccess(r *randGen, call *BpfCall, arg int) (*BpfArg, bool) {
	...
	for i := 0; i < 5; i++ {
		...
		if len(p.pt.User) > 6 && p.pt.User[0:6] == "struct" {
			...
			if !injectGenRandBpfCtxAccess && !readAccess && !writeAccess {
				continue
			}
		} else {
			...
			if !injectGenRandBpfCtxAccess && !defaultAccess.canRead && !defaultAccess.canWrite {
				continue
			}
		}
...
func bpfRetType(call *BpfCall) string {
	if call.Helper.Ret == "RET_INTEGER" {
		...
		call.Helper.Ret == "RET_PTR_TO_MAP_VALUE_OR_NULL" && call.ArgMap != nil {
...
func bpfRetTypeShuffle(call *BpfCall) string {
	if call.Helper.Ret == "RET_INTEGER" {
		return "uint64_t"
	} else if call.Helper.Ret == "RET_PTR_TO_MAP_VALUE" ||
...
func (p *BpfProg) genBpfHelperCall(r *randGen, helper *BpfHelper, hint *BpfCallGenHint, prepend bool) (*BpfCall, bool) {
	...
	if injectShuffleReturnType {
		if typ := bpfRetTypeShuffle(call); typ != "" {
			call.RetType = typ
		}
	} else {
...
func (p *BpfProg) genRandBpfHelperCall(r *randGen, call *BpfCall, arg int) (*BpfArg, bool) {
	...
	if ok {
		...
		if ok && !injectGenRandBpfHelperCall {
...
func (brf *BpfRuntimeFuzzer) GenBpfProg(r *randGen, opt BrfGenProgOpt, injector string) (*BpfProg, bool) {
	pt := brf.progTypeMap[BpfProgTypeEnum(r.Intn(int(BPF_PROG_TYPE_TRACING))+1)]
	helper := pt.Helpers[r.Intn(len(pt.Helpers))]
	p := NewBpfProg(pt, r, opt, injector)
	if verbose {
		fmt.Printf("gen prog %v %v %v\n", pt.Name, helper.Enum, helper.Uname)
	}
	rd = 0
	hint := newBpfCallGenHint(nil)
		...
		if len(constraints) != 0 {
			...
			for i, c := range constraints {
				if injectShuffleBoundChecksOperator {
					c = randomizeComparator(c)
					//fmt.Println(c)
				}
				if injectOffsetBoundCheck {
					num := 0
					c, num = shiftRHS(c)
					trail = fmt.Sprintf(" //offset=%d", num)
				}
				fmt.Fprintf(s, "%v", c)
